...
export function exportHistoryCSV(history) {
  const header = 'Modo,Valor (R$),Distância Total (km),Desloc. Coleta (km),Rota Entrega (km),Lucro (R$),Horário';
  const rows = history.map(h => {
    const modo = h.mode === 'ml' ? 'Mercado Livre' : 'Lalamove / inDrive';
    ...
    const distColeta = (h.distanciaColeta || 0).toFixed(1);
    const distRota = (h.distanciaRota || h.distancia || 0).toFixed(1);
    return `${modo},${h.valor.toFixed(2)},${distTotal},${distColeta},${distRota},${h.lucro.toFixed(2)},${hora}`;
  });
